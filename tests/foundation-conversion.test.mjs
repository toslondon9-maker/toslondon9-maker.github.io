...
test("renderWhatHappensNext renders three ordered translated steps and the supplied start link", () => {
  ...
  assert.match(html, /^<section[^>]+class="conversionJourney"/);
...
test("renderFoundationNextStep renders the supplied canonical offer and safe external payment link", () => {
  ...
  assert.match(html, /^<section[^>]+class="foundationNextStep"/);
  ...
  for (const key of ["conversion.foundation.heading", "conversion.foundation.price", "conversion.foundation.payment", "conversion.foundation.coaching"]) assert.match(html, new RegExp(`data-i18n="${key}"`));
});
