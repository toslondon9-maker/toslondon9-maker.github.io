...
export function resourcesPage(data = siteData, language = "en") {
  const listenAudio = audioSection(language);
  const groups = [
    ...
    ["LISTEN", "Audio, meditation and focus material.", `<a href="${data.routes.startFree}">Use the guided daily practice</a>${listenAudio}`, "resourcesPage__group--listen"],
