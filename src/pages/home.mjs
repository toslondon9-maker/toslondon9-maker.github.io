...
export function renderHome({ language = "en" } = {}) {
  return renderHomeBody({ language }).replace(
    ...
    `</section>${renderWhatHappensNext({ language, data: siteData, startHref: siteData.routes.startFree })}<section class="homeSection homeLineage"`,
  );
}
