...
export function faqPage(data = siteData, language = "en") {
  const items = faqItems.map((number) => { const answer = t(`coaching.faq.${number}.answer`, language); const rendered = number === 7 ? `<span data-i18n="coaching.faq.7.answerBeforeLink">${esc(t("coaching.faq.7.answerBeforeLink", language))}</span><a href="${data.routes.refundPolicy}" data-i18n="coaching.faq.7.answerLink">${esc(t("coaching.faq.7.answerLink", language))}</a><span data-i18n="coaching.faq.7.answerAfterLink">${esc(t("coaching.faq.7.answerAfterLink", language))}</span>` : esc(answer); return `<details><summary data-i18n="coaching.faq.${number}.question">${esc(t(`coaching.faq.${number}.question`, language))}</summary><p>${rendered}</p></details>`; }).join("");
  const key = "route.faq";
