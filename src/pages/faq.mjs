...
export function faqPage(data = siteData, language = "en") {
  const items = faqItems.map((number) => { const answer = t(`coaching.faq.${number}.answer`, language); const rendered = number === 7 ? answer.replaceAll("/refund-policy/", data.routes.refundPolicy) : esc(answer); return `<details><summary data-i18n="coaching.faq.${number}.question">${esc(t(`coaching.faq.${number}.question`, language))}</summary><p data-i18n="coaching.faq.${number}.answer">${rendered}</p></details>`; }).join("");
  const key = "route.faq";
