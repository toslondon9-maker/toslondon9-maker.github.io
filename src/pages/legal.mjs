...
function privacyContent(data) {
  return [
    ...
    section("Seven-Day Registration", `<p>When you register for the Free 7-Day Experience, we collect your first name, surname, email address, WhatsApp number, goals, current difficulty and required WhatsApp-contact consent. We also record your yes-or-no choice about optional email marketing; it is not required. The registration is processed through a dedicated service and stored in a private Google Sheet so Tariq can deliver relevant follow-up.</p>`),
