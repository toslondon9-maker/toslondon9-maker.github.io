...
function markdownToHtml(markdown, { skipCtas = false, richMarkdown = false } = {}) {
  ...
  for (const line of lines) {
    ...
    if (line.startsWith("# ")) { flushBlocks(); if (firstHeading) { firstHeading = false; continue; } out.push(`<h1>${inline(line.slice(2))}</h1>`); continue; }
...
export function sourceArticlePage({ data, language = "en", id, metaKey, pdf }) {
  ...
  const publicationDate = metaKey === "insights.howToStudy" ? "2026-09-25" : ["insights.haanelBiography", "insights.powerWithin"].includes(metaKey) ? "2026-09-19" : ["insights.people", "insights.foundationDevelopment", "insights.foundationFirstStep", "insights.foundationQA"].includes(metaKey) ? "2026-09-11" : ["insights.personalCoaching", "insights.imagineMeditation"].includes(metaKey) ? "2026-09-13" : "2026-09-10";
  const publicationKey = publicationDate === "2026-09-25" ? "insights.publicationDateHowToStudy" : publicationDate === "2026-09-19" ? "insights.publicationDateHaanel" : publicationDate === "2026-09-13" ? "insights.publicationDatePersonal" : publicationDate === "2026-09-11" ? "insights.publicationDatePeople" : "insights.publicationDate";
  ...
  const disclaimerKey = metaKey === "insights.powerWithin" ? "insights.powerWithin.disclaimer" : "insights.source.disclaimer";
