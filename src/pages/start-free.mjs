...
function renderRegistration(language) {
  ...
  return `<section class="sevenDayRegistration"><div class="sevenDayDashboard__inner"><h2>${copy[0]}</h2><form data-lead-capture-form data-lead-endpoint="${leadCaptureConfig.endpoint ?? ""}"><label for="lead-first-name" data-lead-label="first">${copy[1]}</label><input id="lead-first-name" name="firstName" autocomplete="given-name" maxlength="80" required><label data-lead-label="last">${copy[2]}<input name="surname" autocomplete="family-name" maxlength="80" required></label><label data-lead-label="email">${copy[3]}<input name="email" type="email" autocomplete="email" maxlength="254" required></label><label data-lead-label="whatsapp">${copy[4]}<input name="whatsapp" type="tel" autocomplete="tel" maxlength="32" required></label><label data-lead-label="goal">${copy[5]}<textarea name="goal" maxlength="1000" required></textarea></label><label data-lead-label="difficulty">${copy[6]}<textarea name="difficulty" maxlength="1000" required></textarea></label><label data-lead-label="consent"><input type="checkbox" name="consent" required> ${copy[7]}</label><label data-lead-label="marketing"><input type="checkbox" name="emailMarketing"> ${copy[8]}</label><input name="website" class="visually-hidden" tabindex="-1" autocomplete="off"><p><a href="/privacy/">Privacy Policy</a></p><p data-lead-capture-status role="status" aria-live="polite" tabindex="-1">WhatsApp</p><button type="submit" class="button--primary">${copy[9]}</button></form><div data-lead-capture-success hidden><p>You’re registered. Welcome to Unleash Your Power.</p><a class="button--primary" href="/start-free/day-1-see-whats-running-your-life/">START DAY 1</a></div></div></section>`;
}
