...
function renderLessonCards(language) {
  return sevenDayExperience.lessons.map((lesson) => {
    ...
    return `<li class="sevenDayDashboard__lesson" data-progress-lesson="${lesson.id}"><a href="${escapeHtml(lesson.route)}"><span class="sevenDayDashboard__number" aria-hidden="true">${String(lesson.sequence).padStart(2, "0")}</span><span class="sevenDayDashboard__lessonCopy"><span class="sevenDayDashboard__status" data-i18n="${contentKeys.status}">${escapeHtml(t(contentKeys.status, language))}</span><strong data-i18n="${contentKeys.title}">${escapeHtml(t(contentKeys.title, language))}</strong><small class="sevenDayDashboard__time" data-i18n="sevenDay.lessons.day${lesson.sequence}.time">${escapeHtml(t(`sevenDay.lessons.day${lesson.sequence}.time`, language))}</small><span class="sevenDayDashboard__status" data-progress-marker hidden data-i18n="${completionKey}">${escapeHtml(t(completionKey, language))}</span><span class="sevenDayDashboard__completionMessage" data-progress-completion-message hidden></span></span><span class="sevenDayDashboard__arrow" aria-hidden="true">→</span></a></li>`;
  }).join("");
}
...
function renderConversionValue(language) {
  const benefits = Array.from({ length: 7 }, (_, index) => `<li data-i18n="sevenDay.conversion.benefit${index + 1}">${escapeHtml(t(`sevenDay.conversion.benefit${index + 1}`, language))}</li>`).join("");
  return `<section class="sevenDayConversion" aria-labelledby="seven-day-conversion-title"><div class="sevenDayDashboard__inner"><h2 id="seven-day-conversion-title" data-i18n="sevenDay.conversion.title">${escapeHtml(t("sevenDay.conversion.title", language))}</h2><p class="sevenDayConversion__intro" data-i18n="sevenDay.conversion.intro">${escapeHtml(t("sevenDay.conversion.intro", language))}</p><h3 data-i18n="sevenDay.conversion.benefitsHeading">${escapeHtml(t("sevenDay.conversion.benefitsHeading", language))}</h3><ul class="sevenDayConversion__benefits">${benefits}</ul><p class="sevenDayConversion__commitment" data-i18n="sevenDay.conversion.commitment">${escapeHtml(t("sevenDay.conversion.commitment", language))}</p><p class="sevenDayConversion__reassurance" data-i18n="sevenDay.conversion.reassurance">${escapeHtml(t("sevenDay.conversion.reassurance", language))}</p><a class="button--primary sevenDayConversion__cta" href="#start-free-registration" data-start-free-cta data-i18n="sevenDay.conversion.cta">${escapeHtml(t("sevenDay.conversion.cta", language))}</a><p class="sevenDayConversion__ctaNote" data-i18n="sevenDay.conversion.ctaNote">${escapeHtml(t("sevenDay.conversion.ctaNote", language))}</p></div></section>`;
}
...
function renderRegistrationMarkup(language) {
  ...
  const copy = es ? { heading: "Comienza tu experiencia gratuita de 7 días", helper: "Regístrate una vez para abrir tu panel privado de siete días.", first: "Nombre", last: "Apellido", email: "Correo electrónico", whatsapp: "Número de WhatsApp con código de país", goal: "¿Qué te gustaría cambiar o mejorar ahora mismo? (opcional)", difficulty: "¿Qué te está frenando más? (opcional)", consent: "Acepto que Tariq me contacte por WhatsApp sobre la experiencia gratuita de 7 días.", marketing: "Me gustaría recibir novedades ocasionales por correo electrónico.", privacy: "Política de privacidad", submit: "COMENZAR MIS 7 DÍAS GRATIS", success: "Tu registro se ha completado. Bienvenido a Unleash Your Power.", successPrompt: "Elige cómo te gustaría continuar.", successAction: "COMPLETAR ONLINE", successDownload: "DESCARGAR EL CUADERNO (PDF)", successNote: "Puedes usar una opción, o ambas.", placeholders: ["Tu nombre", "Tu apellido", "tu@ejemplo.com", "+34611223345", "¿Qué te gustaría mejorar?", "¿Qué lo está dificultando ahora?"] } : { heading: "Begin Your Free 7-Day Experience", helper: "Register once to open your private seven-day dashboard.", first: "First name", last: "Surname", email: "Email", whatsapp: "WhatsApp number including country code", goal: "What would you most like to change or improve right now? (optional)", difficulty: "What is currently holding you back most? (optional)", consent: "I agree that Tariq may contact me on WhatsApp about the Free 7-Day Experience.", marketing: "I would like occasional email updates.", privacy: "Privacy Policy", submit: "START MY FREE 7 DAYS", success: "You’re registered. Welcome to Unleash Your Power.", successPrompt: "Choose how you’d like to continue.", successAction: "COMPLETE ONLINE", successDownload: "DOWNLOAD WORKBOOK (PDF)", successNote: "You can use either option, or both.", placeholders: ["Your first name", "Your surname", "you@example.com", "+34611223345", "What would you like to improve?", "What is making that difficult right now?"] };
  const successFree = t("leadCapture.success.free", language);
  const successNextSteps = t("leadCapture.success.nextSteps", language);
  const successWhatsapp = t("leadCapture.success.whatsapp", language);
  const whatsappHref = bookingCallHref(siteData.contact.whatsapp);
  return `<section class="sevenDayRegistration"><div class="sevenDayDashboard__inner"><h2 data-lead-heading>${copy.heading}</h2><p data-lead-helper>${copy.helper}</p><noscript><p class="sevenDayRegistration__noScript">${noScript} <a href="https://wa.me/34611223345" target="_blank" rel="noopener noreferrer">WhatsApp</a>.</p></noscript><form data-lead-capture-form data-lead-endpoint="${leadCaptureConfig.endpoint ?? ""}" method="post"><label for="lead-first-name"><span data-lead-label="first">${copy.first}</span></label><input id="lead-first-name" name="firstName" autocomplete="given-name" maxlength="80" required data-lead-placeholder="first" placeholder="${copy.placeholders[0]}"><label><span data-lead-label="email">${copy.email}</span><input name="email" type="email" autocomplete="email" maxlength="254" required data-lead-placeholder="email" placeholder="${copy.placeholders[2]}"></label><details data-optional-details><summary data-i18n="sevenDay.registration.optionalDetails">${language === "es" ? "Añadir detalles opcionales" : "Add optional details"}</summary><label><span data-lead-label="last">${copy.last}</span><input name="surname" autocomplete="family-name" maxlength="80" data-lead-placeholder="last" placeholder="${copy.placeholders[1]}"></label><label><span data-lead-label="whatsapp">${copy.whatsapp}</span><input name="whatsapp" type="tel" autocomplete="tel" maxlength="32" data-lead-placeholder="whatsapp" placeholder="${copy.placeholders[3]}"></label><label><span data-lead-label="goal">${copy.goal}</span><textarea name="goal" maxlength="1000" data-lead-placeholder="goal" placeholder="${copy.placeholders[4]}"></textarea></label><label><span data-lead-label="difficulty">${copy.difficulty}</span><textarea name="difficulty" maxlength="1000" data-lead-placeholder="difficulty" placeholder="${copy.placeholders[5]}"></textarea></label><label><input type="checkbox" name="consent"><span data-lead-label="consent">${copy.consent}</span></label><label><input type="checkbox" name="emailMarketing"><span data-lead-label="marketing">${copy.marketing}</span></label></details><p><a href="/privacy/" data-lead-privacy-link>${copy.privacy}</a></p><p data-lead-capture-status role="status" aria-live="polite" tabindex="-1"></p><p class="sevenDayRegistration__notice" data-i18n="sevenDay.registration.notice">${language === "es" ? "Regístrate una vez y empieza el día 1 inmediatamente. Tu progreso permanece privado en tu dispositivo." : "Register once, then begin Day 1 immediately. Your lesson progress remains private on your device."}</p><button type="button" data-lead-submit class="button--primary" disabled>${copy.submit}</button></form><div class="sevenDayRegistration__success" data-lead-capture-success hidden><p data-lead-success>${copy.success}</p><p data-lead-success-free data-i18n="leadCapture.success.free">${escapeHtml(successFree)}</p><p data-lead-success-prompt>${copy.successPrompt}</p><p data-lead-success-next-steps data-i18n="leadCapture.success.nextSteps">${escapeHtml(successNextSteps)}</p><p class="sevenDayRegistration__supporting" data-i18n="sevenDay.success.supporting">${escapeHtml(t("sevenDay.success.supporting", language))}</p><div class="sevenDayRegistration__successActions"><a class="button--primary" data-lead-success-action href="/start-free/day-1-see-whats-running-your-life/" data-i18n="sevenDay.success.day1">${escapeHtml(t("sevenDay.success.day1", language))}</a><a class="button--secondary" data-lead-success-download href="/downloads/seven-day-experience-workbook-en.pdf" download>${copy.successDownload}</a><a class="button--secondary sevenDayRegistration__successWhatsapp" data-lead-success-whatsapp href="${escapeHtml(whatsappHref)}" target="_blank" rel="noopener noreferrer">${escapeHtml(successWhatsapp)}</a></div><p data-lead-success-note>${copy.successNote}</p></div></div></section>`;
}

function renderRegistration(language) {
  return renderRegistrationMarkup(language).replace(
    '<section class="sevenDayRegistration">',
    '<section class="sevenDayRegistration" id="start-free-registration"><input name="website" class="visually-hidden" tabindex="-1" autocomplete="off">',
  );
}

function renderStartFreeBody({ language = "en" } = {}) {
...
export function startFreePage(data = siteData, language = "en") {
  return {
    ...
    scripts: ["/assets/lead-capture-form.mjs", "/assets/seven-day-progress.mjs", "/assets/start-free-conversion.mjs", "/assets/flyer-lightbox.mjs"],
