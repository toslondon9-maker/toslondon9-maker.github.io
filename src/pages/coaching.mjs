...
function flagshipCoaching(language, route) {
  ...
  return `<section class="coachingFlagship section" data-coaching-section="flagship"><div class="coachingFlagship__intro"><p class="eyebrow">FLAGSHIP OFFER</p><h2>Personal Master Key Coaching</h2><p>Personal guidance, accountability and practical support as you study and apply Charles F. Haanel’s Master Key System in real life.</p><p><strong>How coaching works:</strong> focused conversations with Tariq connect the lesson you are studying to reflection, practical application and your next clear action.</p></div><ul class="coachingFlagship__benefits"><li><strong>Who coaching is for</strong><span>People who want a steady guide as they turn study into a lived practice.</span></li><li><strong>What you receive</strong><span>Focused conversations, structured reflection and support for the lesson in front of you.</span></li><li><strong>Accountability and practical application</strong><span>Stay engaged with the commitments you choose and apply the principles to real decisions and daily life.</span></li></ul><a class="button--primary" href="${route}">ENQUIRE ABOUT COACHING</a></section>`;
}
...
export function renderCoaching({ language = "en", siteData = canonicalSiteData } = {}) {
  ...
  return `<main class="coachingPage" id="main-content"><header class="coachingHero section"><p class="eyebrow">${copy("coaching.eyebrow", language)}</p><h1 data-i18n="route.coaching.heading">${escapeHtml(t("route.coaching.heading", language))}</h1><p class="routeShell__purpose" data-i18n="route.coaching.purpose">${escapeHtml(t("route.coaching.purpose", language))}</p><a class="button--primary routeShell__action" href="${siteData.routes.contact}" data-i18n="route.coaching.action">${escapeHtml(t("route.coaching.action", language))}</a><p class="coachingHero__preparation">Prepare for Week 1 with your own study copy: <a href="${siteData.routes.getTheBook}">BUY THE MKS BOOK</a></p></header>${flagshipCoaching(language, siteData.routes.contact)}<section class="coachingExperience section--night"><div class="coachingExperience__inner" data-tabs><header class="coachingExperience__header"><p class="eyebrow">INVESTMENT</p><h2>Choose your 24-week journey</h2><p>Explore the four progressive stages, the complete programme investment and your next step.</p></header><div class="tabs coachingTabs" role="tablist" aria-label="${escapeHtml(t("coaching.tabsLabel", language))}" data-i18n-aria-label="coaching.tabsLabel">${tabs}</div><div class="coachingPanels">${tabPanels}</div></div></section>${professionalServices(language, siteData.routes.contact)}<section class="coachingTrust section"><p class="eyebrow">NEXT STEP</p><h2>Enquire about coaching</h2><p>${copy("coaching.trust.body", language)}</p><a class="button--primary" href="${siteData.routes.contact}">ENQUIRE ABOUT COACHING</a></section></main>`;
}
