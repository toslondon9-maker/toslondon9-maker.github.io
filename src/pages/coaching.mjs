...
function coachingOutcome() {
  return `<section class="coachingOutcome section" data-coaching-section="outcome"><div class="coachingOutcome__inner"><p class="eyebrow">WHAT YOU CAN BUILD</p><h2>Become the conscious creator of your life.</h2><p>This is a six-month journey into the power within you: a practical, disciplined process for becoming calmer, clearer, more focused and more deliberate in the way you think, feel, choose and act.</p><p>You bring the willingness. Tariq brings structure, timeless Master Key principles, weekly guidance, practical exercises, accountability and support to help you live them.</p><p class="coachingOutcome__quote">“The reading gives you the knowledge. The daily exercise creates the transformation.”</p><p class="coachingOutcome__note">Individual outcomes depend on your circumstances, participation and consistent practice.</p></div></section>`;
}

function coachingRateCard(language, siteData) {
  const offers = [
    ["coaching.rate.free", siteData.routes.startFree, ""], ["coaching.rate.foundation", siteData.stages[0].paymentUrl, ""], ["coaching.rate.complete", siteData.offer.paymentUrl, "coachingRateCard__featured"], ["coaching.rate.mastery", siteData.routes.contact, ""], ["coaching.rate.mentoring", siteData.routes.contact, ""], ["coaching.rate.alumni", siteData.routes.contact, ""], ["coaching.rate.corporate", siteData.routes.contact, ""],
  ];
  return `<section class="coachingRateCard section" data-coaching-section="rate-card"><div class="coachingRateCard__inner"><p class="eyebrow">${copy("coaching.rate.eyebrow", language)}</p><h2>${copy("coaching.rate.title", language)}</h2><div class="coachingRateCard__grid">${offers.map(([key, href, className]) => `<article class="coachingRateCard__item ${className}"><h3>${copy(`${key}.title`, language)}</h3><p class="coachingRateCard__price">${copy(`${key}.price`, language)}</p><p class="coachingRateCard__status">${copy(`${key}.status`, language)}</p><a class="button--${className ? "primary" : "secondary"}" href="${escapeHtml(href)}">${copy("coaching.rate.action", language)}</a></article>`).join("")}</div></div></section>`;
}
