...
const section = (eyebrow, heading, body, extra = "") => `<section class="mksLineage__section"><p class="eyebrow">${eyebrow}</p><h2>${heading}</h2><div class="mksLineage__copy"><p>${body}</p>${extra}</div></section>`;
...
export function mksLineagePage(data = siteData, language = "en") {
  const body = `<main class="mksLineagePage" id="main-content">
    ...
    <div class="mksLineage__content">
      ${section("THE ORIGINAL SOURCE", "Charles F. Haanel — The Original Author", "Charles F. Haanel wrote The Master Key System. The Unleash Your Power programme studies his original ideas and exercises as a structured 24-week journey, with respect for their historical source and meaning.")}
      ...
      ${section("YOUR GUIDE", "Tariq Saddique — Your Guide Through the 24-Week Journey", "Tariq guides students through the weekly lessons, exercises, reflection, practical application and accountability that support consistent study. He is not presenting himself as Charles F. Haanel or Helmar Rudolph; he offers his own independent Unleash Your Power study and coaching programme.")}
