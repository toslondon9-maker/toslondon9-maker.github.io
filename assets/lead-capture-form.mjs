...
export function localizeForm(form, language, documentRef = globalThis.document) { const copy = labels[language === "es" ? "es" : "en"]; for (const label of form.querySelectorAll("[data-lead-label]")) { const input = label.querySelector("input, textarea"); if (input) label.firstChild.textContent = `${copy[label.dataset.leadLabel]} `; else label.textContent = copy[label.dataset.leadLabel]; } form.querySelector("[data-lead-capture-status]").textContent = copy.unavailable; form.querySelector("button[type=submit]").textContent = copy.submit; const success = documentRef?.querySelector?.("[data-lead-capture-success] p"); if (success) success.textContent = copy.success; }
