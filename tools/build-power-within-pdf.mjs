...
for (const line of markdown.split(/\r?\n/)) {
  ...
  else if (line.trim() && !/^\**\[(START THE FREE SEVEN-DAY EXPERIENCE|EXPLORE THE MASTER KEY SYSTEM|BOOK A FREE 15-MINUTE CALL)\]\**$/.test(line.trim())) body.push(line.replace(/^[-*]\s+/, "").replace(/^\d+\.\s+/, "").replaceAll("**", "").replaceAll("*", ""));
}
